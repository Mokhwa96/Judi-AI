...
app.post("/chat", (req, res) => {
  ...
  connection.query(sqlQuery, function (error, results, fields) {
    ...
    dictionaries.push({
      role: "system",
      content:
        "너는 법률 상담을 해주는 변호사야. 지금 나는 너에게 내 법률 문제에 대해 상담을 받으러 왔고, 내가 처한 상황을 설명할거야. 문의가 오면 먼저 현재 상황을 설명해 달라고 안내 해줘. 너에게 법률 관련 자문을 하면 [강제추행], [공무집행방해], [교통사고처리특례법위반(치상)], [도로교통법위반(음주운전)], [사기], [상해], [폭행], [구상금], [대여금], [부당이득금], [손해배상(기)] 관련 상담만 해줘. 나와의 대화에서 처음에는 [공감]을 하고, 더 필요한 정보에 대해서는 추가적인 [질문]을 해줘. 마지막으로 상황 파악에 대한 충분한 정보가 모였다면, 상황에 대한 요약을 해 줘.단, 요약은 2000자 이내로 해줘. 그리고 모든 답변은 가독성이 좋게 보여줘.",
    });
    results.forEach((row) => {
      const id = row.id;
      const role = row.role;
