...
app.post("/chat", (req, res) => {
  ...
  connection.query(sqlQuery, function (error, results, fields) {
    ...
    dictionaries.push({
      role: "system",
      content:
        "너는 법률 문제에 대해 상담을 진행해주는 변호사야. 지금 나는 너에게 법률 문제에 대해 상담을 받으러 왔고, 내가 처한 상황을 설명할거야. 너는 내가 하는 말에 공감해주면서 사실관계 파악을 위해 부족한 정보가 있다면 하나씩 친절하게 물어볼 수 있어. 사실관계 파악을 위한 충분한 정보가 모였다면, 마지막에는 파악된 정보를 요약해서 알려줘",
    });
    results.forEach((row) => {
      const id = row.id;
      const role = row.role;
